        ...
        {data?.map(invoice =>(
          <Link href={`/invoices/${invoice.id}`} passRef key={invoice.id}>
            <div className="invoice__item">
              <div>
                <h5 className="invoice__id">{invoice.id.substr(0, 6).toUpperCase()}
                </h5>
              </div>
...
export async function getStaticProps() {
  const client = await MongoClient.connect('mongodb+srv://inadmin:adminpass@invoice1.jgk5yn8.mongodb.net/invoices1?retryWrites=true&w=majority',{ useNewUrlParser: true }
  );
